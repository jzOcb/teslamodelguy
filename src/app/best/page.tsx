...
export default function BestPage() {
  return (
    <div className="max-w-6xl mx-auto px-4 py-16">
      ...
      <p className="text-gray-600 mb-8">Our curated recommendations — only products we actually stand behind.</p>
      <div className="space-y-4">
        {articles.map((a) => (
          <Link key={a.slug} href={a.slug} className="block border border-gray-200 rounded-xl p-6 hover:border-blue-300 hover:shadow-md transition">
            <span className="text-xs font-medium text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">{a.tag}</span>
            <h2 className="text-xl font-bold text-gray-900 mt-2">{a.title}</h2>
